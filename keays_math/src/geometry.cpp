...
//--------------------------------------------------------------
KEAYS_MATH_EXPORTS_API bool
GenPointHeight(const keays::types::VectorD3 &center, const double &fallAngle, const double &fallPct, keays::types::VectorD3 &pt, tGradeSegmentList *p_cGrades)
{
    // get angle to each pt ensure it is greater than the fall angle
    double ang, angDiff, dist, fall, deltaHeight;
    bool haveCrossGrades = false;
                              ...
                              bool averageVectors /*= true*/, bool isClosed /*= false*/)
{
    ...
    // for each segment of a keays::types::Polyline
    std::vector<int> indices;
    int idx = 0;

    keays::types::VectorD3 perpVector;

    keays::types::Polyline3D tmpPerpVectors;
    keays::types::Polyline3D::const_iterator itr, prevItr;
...
InsertPoint(keays::types::Polyline3D *pPolyline, const double &chainage, size_t *pInsertIndex /*= NULL*/)
{
    ...
    double grade = 0.0;
    keays::types::VectorD2 normal;
    size_t insertIndex = 0;

    keays::types::Polyline3D::iterator itr(pPolyline->begin()), nextItr(itr);
    nextItr++;
...
InsertPoint(keays::types::Polyline2D *pPolyline, const double &chainage, size_t *pInsertIndex /*= NULL*/)
{
    ...
    keays::types::Polyline2D::iterator itr(pPolyline->begin()), nextItr(itr);
    nextItr++;
             ...
             const double &interval, int *pNumPtsAdded /*= NULL*/)
{
    ...
    int count = 0;
    double length = 0.0;
    keays::types::Polyline3D result;

    keays::types::Polyline3D::const_iterator itr(srcPolyline.begin()), nextItr(itr);
    nextItr++;
...
InsertPoints(const keays::types::Polyline2D &srcPolyline, keays::types::Polyline2D *pDestPolyline, const double &interval, int *pNumPtsAdded /*= NULL*/)
{
    ...
    int count = 0;
    double length = 0.0;
    keays::types::Polyline2D result;

    keays::types::Polyline2D::const_iterator itr(srcPolyline.begin()), nextItr(itr);
    nextItr++;
...
{
    keays::types::VectorD2 rPt;

    double deltaX, deltaY, len;
    double cosTheta, sinTheta;

    if ((base.x == ref.x) && (base.y == ref.y))
        return keays::types::VectorD2(-999, -999);
...
PointOnSide(const keays::types::VectorD3 &start, const keays::types::VectorD3 &end, const keays::types::VectorD3 &pt, const double &tolerance /*= Float::TOLERANCE*/, double *pDistance /*= NULL*/)
{
...
//--------------------------------------------------------------
KEAYS_MATH_EXPORTS_API const eSideSelections
PointOnSide(const keays::types::VectorD2 &start, const keays::types::VectorD2 &end, const keays::types::VectorD2 &pt, const double &tolerance /*= Float::TOLERANCE*/, double *pDistance /*= NULL*/)
{
...
//--------------------------------------------------------------
// Calculate the 2D Length of a keays::types::Polyline.
KEAYS_MATH_EXPORTS_API const double
...
//--------------------------------------------------------------
// Calculate the 3D Length of a keays::types::Polyline.
KEAYS_MATH_EXPORTS_API const double
...
//--------------------------------------------------------------
// Normalise a keays::types::VectorD3
KEAYS_MATH_EXPORTS_API const keays::types::VectorD3
NormaliseD(const keays::types::VectorD3 vec)
{
    double mag = Magnitude(vec);

    if (0 == mag)
        return keays::types::VectorD3(0, 0, 0);

    return keays::types::VectorD3(vec.x / mag, vec.y / mag, vec.z / mag);
}

//--------------------------------------------------------------
// Normalise the Vector given by the two points specified
KEAYS_MATH_EXPORTS_API const keays::types::VectorD3
NormaliseD(const keays::types::VectorD3 base, const keays::types::VectorD3 tip)
{
    keays::types::VectorD3 vec((base.x - tip.x), (base.y - tip.y), (base.z - tip.z));
    double mag = Magnitude(vec);
...
//--------------------------------------------------------------
// Calculate the crossproduct of 2 keays::types::VectorD3's
KEAYS_MATH_EXPORTS_API const keays::types::VectorD3
...
//--------------------------------------------------------------
// Calculate the Dotproduct of 2 keays::types::VectorD2's
KEAYS_MATH_EXPORTS_API const double
...
//--------------------------------------------------------------
// Calculate the Dotproduct of 2 keays::types::VectorD3's
KEAYS_MATH_EXPORTS_API const double
...
GetPerpendicularIntersect(const keays::types::VectorD2 &lnStart, const keays::types::VectorD2 &lnEnd, const keays::types::VectorD2 &pt, keays::types::VectorD2 &result, double *pDistance)
{
    double dist, lnAng, lnPtAng;
...
/*
    ...
    squared.
    polygon [In]  - a constant reference to a keays::types::Polyline2D points describing a 2D polygon.
    */
KEAYS_MATH_EXPORTS_API const double
                      ...
                      keays::types::VectorD2 *result2)
{
    ...
    result = keays::types::BAD_POINT;
    if (result2)
        *result2 = keays::types::BAD_POINT;
    //
...
//--------------------------------------------------------------
KEAYS_MATH_EXPORTS_API const int
LineLineIntersect(const keays::types::VectorD2 &l1p1, const keays::types::VectorD2 &l1p2, const keays::types::VectorD2 &l2p1, const keays::types::VectorD2 &l2p2, keays::types::VectorD2 &result)
{
    double dx1, dy1;
    double dx2, dy2;
    double denom, top, s;
...
MultipleLineIntersectAverage(const std::vector<Line> &lines, keays::types::VectorD2 *result)
{
    ...
    size_t i, j;
    int rc;
    keays::types::Polyline2D intersects;
...
KEAYS_MATH_EXPORTS_API const double *
YfromXPt(const keays::types::VectorD2 &pt1, const keays::types::VectorD2 &pt2, const double &x, double *pY, const double &tolerance /*= Float::TOLERANCE*/)
{
    assert(pY != NULL);
...
KEAYS_MATH_EXPORTS_API const double *
XfromYPt(const keays::types::VectorD2 &pt1, const keays::types::VectorD2 &pt2, const double &y, double *pX, const double &tolerance /*= Float::TOLERANCE*/)
{
    assert(pX != NULL);
                ...
                keays::types::VectorD2 *pLineEnd, const double &tolerance /*= Float::TOLERANCE*/)
{
    ...
    bool touchesLeft = keays::types::Float::Less(yLeft, rTop) && keays::types::Float::GreaterOrEqual(yLeft, rBottom);
    bool touchesTop = keays::types::Float::GreaterOrEqual(xTop, rLeft) && keays::types::Float::Less(xTop, rRight);
    bool touchesRight = keays::types::Float::LessOrEqual(yRight, rTop) && keays::types::Float::Greater(yRight, rBottom);
    bool touchesBottom = keays::types::Float::Greater(xBottom, rLeft) && keays::types::Float::LessOrEqual(xBottom, rRight);
    int numSolutions = 0;
               ...
               const keays::types::VectorD2 &testPt, const double &tolerance /*= Float::TOLERANCE*/)
{
    ...
    keays::types::VectorD2 testPtToPt2((pt2 - testPt).GetNormalised());

    return keays::types::Float::GreaterOrEqual(pt1ToTestPt.Dot(testPtToPt2), 0.0, tolerance);
}
...
PointOnSegment(const keays::types::VectorD3 &pt1, const keays::types::VectorD3 &pt2,
               const keays::types::VectorD3 &testPt, const double &tolerance /*= Float::TOLERANCE*/)
{
    keays::types::VectorD3 pt1ToTestPt((testPt - pt1).GetNormalised());
    keays::types::VectorD3 testPtToPt2((pt2 - testPt).GetNormalised());
                     ...
                     const double &tolerance  /*= Float::TOLERANCE*/)
{
    ...
    const keays::types::VectorD2 refs[3] = { triPt1, triPt2, triPt3 };
...
SegmentTriangleIntersect(const keays::types::VectorD3 &triPt1, const keays::types::VectorD3 &triPt2,
                         const keays::types::VectorD3 &triPt3,
                         const Line &line, keays::types::VectorD3 *pIntersect,
                         const double &tolerance /*= Float::TOLERANCE*/)
{
    if (!pIntersect)
        return false;
    ...
    *pIntersect = line.start + r * dir;
    ...
    keays::types::VectorD3 w = *pIntersect - triPt1;
    wu = w.Dot(v0);
    wv = w.Dot(v1);
    d = uv * uv - uu * vv;
...
// TODO: Some kind of non KSR, non MFC, non WX logging system.
    ...
    keays::types::Polyline3D::iterator itr, next;
    next = itr = pts.begin();
    next++;
    idx = startPtIdx = 0;
    endPtIdx = pts.size() - 1;
               ...
               const double &curveLength, const unsigned int flags)
{
    ...
    const keays::types::VectorD3 startPt(intermediate[startIndex]);
    const keays::types::VectorD3 endPt(intermediate[endIndex]);
                        ...
                        const double &distance, const eSideSelections side, const double &zDistance, bool closed /*= false*/)
{
    ...
    double rl;
    int rc;
    keays::types::VectorD2 intersectPoint;
...
//-----------------------------------------------------------------------------
KEAYS_MATH_EXPORTS_API const keays::types::Polyline3D
RemoveDuplicates(const keays::types::Polyline3D &sourcePolyline, const double &tolerance /*= Float::TOLERANCE*/ )
{
    keays::types::Polyline3D result;
...
//-----------------------------------------------------------------------------
KEAYS_MATH_EXPORTS_API const keays::types::Polyline3D *
RemoveDuplicates(keays::types::Polyline3D *sourcePolyline, const double &tolerance /*= Float::TOLERANCE*/)
{
    if (sourcePolyline)
    {
...
//-----------------------------------------------------------------------------
double PerpTestData::ms_tolerance = keays::types::Float::TOLERANCE;
                 ...
                 const double &tolerance /*= Float::TOLERANCE*/)
{
    ...
    keays::types::Polyline3D::const_iterator next = i;
    next++;

    std::list< PerpTestData > testData;
    ...
    keays::types::VectorD3 testPoint3D = testPoint;
    keays::types::VectorD3 lastPoint;
